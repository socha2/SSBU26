...
def plot_results(experiment, results, logger):
    ...
    if precision_col in experiment.results.columns:
        plotter.plot_evaluation_metric_over_replications(
            experiment.results.groupby('model')[precision_col].apply(list).to_dict(),
            'Precision per Replication and Average Precision',
            'Precision'
        )
    else:
        logger.warning(f"Skipping precision plot. Available columns: {list(experiment.results.columns)}")
